...
def get_arguments(base_path):
    ...
    some other hyperparameters can only be changed manually (such as model architecture,dropout,etc.)
...
def run_phase(args, loaded_model_weights_path, phase_num, phase_name):
    ...
    :return path to model weights (pytorch file .pth) acquired by the current training phase
    """
    ...
    model_weights_path = os.path.join(trainer.writer.experiment_folder,
                                      trainer.writer.experiment_title + '_BEST_val_{}.pth'.format(critical_metric))
