...
class Writer():
    ...
    def __init__(self, sets, **kwargs):
        ...
        for name, loss_dict in self.losses.items():
            if loss_dict['is_active']:
                for set2 in sets:
                    print('{}_{}_loss_values'.format(name, set2))
                    print('{}_{}_loss_history'.format(name, set2))
                    setattr(self, '{}_{}_loss_values'.format(name, set2), [])
                    setattr(self, '{}_{}_loss_history'.format(name, set2), [])
    ...
    def accuracy_summary(self, mid_epoch):
        ...
        for subj_name, subj_dict in self.subject_accuracy.items():
            ...
            with open(os.path.join(self.per_subject_predictions, 'iter_{}.txt'.format(self.eval_iter)), 'a+') as f:
                f.write('subject:{} ({})\noutputs: {:.4f}\u00B1{:.4f}  -  truth: {}\n'.format(subj_name, subj_mode,
                                                                                              subj_pred, subj_error,
                                                                                              subj_truth))
    ...
    def register_losses(self, **kwargs):
        self.losses = {'intensity':
                           {'is_active': False, 'criterion': L1Loss(), 'thresholds': [0.9, 0.99],
                            'factor': kwargs.get('intensity_factor')},
                       'perceptual':
                           {'is_active': False, 'criterion': Percept_Loss(**kwargs),
                            'factor': kwargs.get('perceptual_factor')},
                       'reconstruction':
