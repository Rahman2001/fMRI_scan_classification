...
class DataHandler():
    ...
    def create_dataloaders(self):
        ...
        if self.current_split_exists():
            train_names, test_names = self.load_split()  # corrected by Rahman
            train_idx, test_idx = self.convert_subject_list_to_idx_list(train_names, test_names, self.subject_list)  # corrected
        else:
            train_idx, test_idx = self.determine_split_randomly(self.subject_list, **self.kwargs)  # corrected
    ...
    def convert_subject_list_to_idx_list(self, train_names, test_names, subj_list):
    ...
    def determine_split_randomly(self, index_l, **kwargs):
        ...
        S_test = np.setdiff1d(np.arange(S), S_train)  # corrected
        train_idx, test_idx = self.convert_subject_list_to_idx_list(S_train, S_test, self.subject_list)  # corrected
        self.save_split({'train_subjects': S_train, 'test_subjects': S_test})  # corrected
        return train_idx, test_idx

    # the whole method is corrected
    def load_split(self):
        ...
        train_names = subject_order[train_index + 1:]
