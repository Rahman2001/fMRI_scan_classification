...
def get_arguments(base_path, seed):
    ...
    parser.add_argument('--augment', type=str, default={
        'gaussian': {'blur': {'prob': 0, 'sigma': (0.0, 1)}, 'noise': {'prob': 0, 'scale': (0, 0.3)}}})
    ...
    parser.add_argument('--losses', default={
        'intensity': {'is_active': False, 'criterion': L1Loss(), 'thresholds': [0.9, 0.99, 15], 'type': 'absolute',
                      'factor': 1, 'erosion': False},
        'perceptual': {'is_active': False, 'loss_type': 'MSE', 'memory_percent': 0.5, 'method': 'repeat', 'factor': 1,
                       'layers': [True, True, False, False]},
        'reconstruction': {'is_active': False, 'criterion': L1Loss(), 'factor': 1},
        'EYESTATUS': {'is_active': False, 'criterion': BCELoss(), 'factor': 1},
        'AGE': {'is_active': False, 'criterion': L1Loss(), 'factor': 1}})

    parser.add_argument('--cuda', default=True)
    parser.add_argument('--workers', default=2)
    parser.add_argument('--finished', default=False)
    parser.add_argument('--stride', default={'sample_stride_factor': 1, 'sequence_stride': 1, 'sequence_length': 20,
                                             'random_TR': True})
    parser.add_argument('--nvidia_vars',
                        default={'is_nvidia': True, 'model_depth': 4, 'hidden_layers': 2, 'intermediate_vec': 2640,
...
def create_model_and_optimizer(test_loader, args, model_weights, lr):
    dim = test_loader.dataset.dataset.get_input_shape()
    if args.task.lower() == 'fine_tune':
        model = Encoder_Transformer_finetune(dim, **args)
    if args.cuda:
        model = model.cuda()

    lr_handler = LrHandler(**args)
    lr = lr_handler.base_lr
    params = model.parameters()
    weight_decay = args.weight_decay
    optimizer = torch.optim.Adam(params, lr=lr, weight_decay=weight_decay)
    lr_handler.set_schedule(optimizer)
...
def run(old_args, base_path, cuda_num):
    ...
    _, _, test_loader = DataHandler(args, test=True).create_dataloaders()
    ...
    model, _ = create_model_and_optimizer(test_loader, args, model_weights, args.lr['base_lr'])
